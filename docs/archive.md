# Block Runner — Design Archive

Closed design decisions, milestone history, and the full decisions log. Reference-only — do **not** treat anything here as still-open. The active working doc is [`design.md`](design.md); the byte-level spec is [`mapping-rules.md`](mapping-rules.md).
...
| M3 | Full mapping | 🚧 in progress | Difficulty scaling; waves; loot biases; 5 categories; 4 enemy types with aggression tiers. **Shipped:** difficulty scaling (HP / damage / speed multipliers from `block.bits`, log curve with caps). **Pending:** 4 enemy types + aggression tiers, loot system + nonce category biases. |
...
- **2026-04-29** — M3 difficulty scaling shipped. `src/game/difficulty.ts` computes `log10(difficulty)` directly from `block.bits` (compact target encoding) using log-space arithmetic — no BigInt, no huge-integer materialisation. `difficultyMultipliers(bits)` returns capped multipliers per the mapping-rules §2 curves: HP up to 8× (`k=0.5`), damage up to 3× (`k=0.15`), speed up to 1.5× (`k=0.035`). `Enemy` stats are now instance-driven (`hp` / `contactDamage` / `speed`) and `applyDifficulty(mults)` is called on each spawn from `ArenaScene.spawnWave`. Player-vs-enemy contact damage now reads off the enemy instance, not the constant. Era / mood post-FX, fog, particles, palette, and visibility are all unaffected — this commit only touches enemy stats.
- **2026-04-29** — Time-of-day modulator shipped (M2 phase 2). `timeOfDayFromTimestamp(ts)` wraps `ts mod 86400` into [0,1); `computeTodTint(tod)` interpolates between four anchor stops (midnight / sunrise / noon / sunset) with shortest-path hue interpolation. Renders as a screen-tint rectangle at depth 70, above particles (60), below the HUD camera; the active mood shader runs over the result. Genesis (mined 18:15 UTC) lands at tod ≈ 0.76, so it now reads warm and golden — fitting for the iconic block.
- **2026-04-29** — Drifting particles shipped (M2 phase 2). Phaser particle emitter with 8–80 particles alive at a time (linearly scaling with `epoch.particleDensity`), random spawn across the arena, slow drift (4–14 px/s), 4–8 s lifespan with a `0 → 0.4 → 0` sine-faded alpha so each particle fades in, peaks, and fades out. Tinted by `palette.particle`, drawn at depth 60 above fog so they read as floating ambient air. Skipped entirely when density < 0.05.
- **2026-04-29** — M2 phase 2 closed: era post-process fade. Continuous linear fade (genesis = 1.0, 17 years past genesis = 0). Implemented as a second post-FX pipeline stacked after the active mood shader on the camera so the layer order from the design (epoch base → time-of-day overlay → mood shader → era filter) is faithfully preserved on the GPU. EraPipeline applies a cream-toned wash on luminance, mild desaturation, animated film grain, faint horizontal scanlines, and a soft vignette, each scaled by intensity so modern blocks pass through unchanged.
